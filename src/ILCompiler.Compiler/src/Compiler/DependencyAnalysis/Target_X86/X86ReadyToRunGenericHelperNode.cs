    ...
    partial class ReadyToRunGenericHelperNode
    {
        protected Register GetContextRegister(ref /* readonly */ X86Emitter encoder)
        {
            if (_id == ReadyToRunHelperId.DelegateCtor)
                return encoder.TargetRegister.Arg1;
            else
        ...
        protected void EmitDictionaryLookup(NodeFactory factory, ref X86Emitter encoder, Register context, Register result, GenericLookupResult lookup, bool relocsOnly)
        {
            ...
            switch (lookup.LookupResultReferenceType(factory))
            {
                case GenericLookupResultReferenceType.Indirect:
                    ...
                    loadEntry = new AddrMode(result, null, 0, 0, AddrModeSize.Int32);
                    encoder.EmitMOV(result, ref loadEntry);
                    break;
                ...
                case ReadyToRunHelperId.GetThreadStaticBase:
                    {
                        ...
                        if (factory.TypeSystemContext.HasLazyStaticConstructor(target))
                        {
                            ...
                            GenericLookupResult nonGcRegionLookup = factory.GenericLookup.TypeNonGCStaticBase(target);
                            EmitDictionaryLookup(factory, ref encoder, encoder.TargetRegister.Arg0, encoder.TargetRegister.Arg1, nonGcRegionLookup, relocsOnly);
                            int cctorContextSize = NonGCStaticsNode.GetClassConstructorContextStorageSize(factory.Target, target);
                            AddrMode loadCctor = new AddrMode(encoder.TargetRegister.Arg1, null, -cctorContextSize, 0, AddrModeSize.Int32);
                            encoder.EmitLEA(encoder.TargetRegister.Arg1, ref loadCctor);
                ...
                case ReadyToRunHelperId.DelegateCtor:
                    {
                        ...
                        // method expects. Codegen also passed us the generic context in Arg1.
                        // We now need to load the delegate target method into Arg1 (using a dictionary lookup)
                        // and the optional 4th parameter, and call the ctor.

                        Debug.Assert(contextRegister == encoder.TargetRegister.Arg1);

                        var target = (DelegateCreationInfo)_target;

                        EmitDictionaryLookup(factory, ref encoder, encoder.TargetRegister.Arg1, encoder.TargetRegister.Arg1, _lookupSignature, relocsOnly);

                        if (target.Thunk != null)
                        {
                            Debug.Assert(target.Constructor.Method.Signature.Length == 3);
                            encoder.EmitLEAQ(encoder.TargetRegister.Arg0, target.Thunk);
                        }
                        else
                        {
